...
numstring1 + as.integer(numstring2)
...
as.integer(gsub(",", "", numstring3)) + numstring1
...
pc1 <- c("V1A2B3", "V1A 2B3", "v1a-2b3", "V1A 2B33")
...
str_sub(fruit, 1, 2)    # starting at the first position (1), end at second position (2)
...
vowel_ex <- str_extract(fruit, "[aeiou]")  # which is the first vowel?

vowel_ma <- str_match(fruit, "(.)[aeiou]") # what is the first letter that comes before a vowel?

fruit_t <- as.tibble(fruit)

fruit_t %>% 
  mutate(first_v = str_match(value, "[aeiou]"))
