...
CSV files are very commonly used for storing flat files. Unlike Excel, they don't have any formatting—just the number or text in the cell.
...
A more reliable approach is to specify exactly the width of each column. Note that in this example, we specify only "name" without splitting it.

```{r}

read_fwf("fwf-sample.txt", fwf_widths(c(20, 10, 12), c("name", "state", "ssn")))
...
```{r}
read_fwf("fwf-sample.txt", fwf_positions(c(1, 30), c(20, 42), c("name", "ssn")))
```
...
read_fwf("fwf-sample2.txt", fwf_cols(name = c(1, 20), ssn = c(30, 42)))
...
There are X different variables, described in the table below:
...
_Using what you've learned from the names file "fwf-sample.txt" above, read the file "penguins_fwf.txt" into R. Assign the "sex" variable as a factor, and "year" as an integer._
