...
It is an unfortunate truth that many organizations make data available as part of a PDF report. This file format was developed to create documents that print on paper as they appear on the screen, and have a great deal of functionality to create high quality output. 

But for someone who is going to analyze the data, they can be a challenge.

Fortunately for us, other data scientists have already confronted this challenge, and have made tools available for the rest of  us. One such package is [{pdftools}](https://CRAN.R-project.org/package=pdftools).
...
Once it's downloaded, we can use the `str()` function to display a summary of the structure of our new object.
...
What we see is that we have a vector with 101 character strings—one for each page of the original document. We can access each page, using the square bracket accessor syntax of R. Here is the code to see the contents of the first page:
