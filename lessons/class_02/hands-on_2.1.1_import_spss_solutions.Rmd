...
If you work with statisticians, economists, sociologists, survey practioners, and many others, you will find yourself encountering data files created using the software packages SAS, SPSS, and Stata. These data files have their own unique attributes, and require some decoding to sort through.
...
Run this code chunk to load the packages:

```{r setup, message = FALSE}
library(tidyverse)  

# 
library(haven)     # {haven} is the tidyverse package for reading spss, sas, and stata files
library(labelled)  # {labelled} is a package that provides functions to work with the metadata that {haven} reads from spss files
```
...
But first, let's read the "iris" data table in from an SAS data file (with the "sas7bdat" extension):
...
Compare all the variable names--in addition to the "@" symbol being replaced by the letter "x", what other changes in variable names do you see?
...
Very often, SPSS files will have "user defined missing values". As we see above, the values in `heardofcondition` that were "N/A" (that is, coded as "0") have been read in as "NA". But often, survey data will have a number of different responses we might want to analyze as "missing values":
...
Depending on the circumstance, you may want to include these responses in your calculations. For example, if there a lot of "Don't know" and "Not applicable" responses, you may wish to analyze the proportions of each (they mean very different things!) If the are all coded as "NA", you have lost that ability to differentiate, and ultimately to understand the responses.
...
mean(df_video$heardofcondition)
