...
The {tidyverse} approach to this is a _tibble_, which is either the phonetic sounding out of the letters "t-b-l" (sometimes used as an extension for a table file), or the spelling of how New Zealanders pronounce the word "table" (there are a few important figures in the R community, including the two professors who first developed the program).

In _R for Data Science_ there is [an entire chapter devoted to tibbles ](https://r4ds.had.co.nz/tibbles.html)

In this example, we will create a tibble with three variables, `x`, `y`, and `z`. `x` will be equal to 2, `y` will be the integers from 1 to 5, and `z` will be `x` + `y` squared.
...
Note that in this case, the variables are listed first and are each identified with a tilde ~, and then each row's contents. In this way, it is visually structured more like the table we want. In this case, we have the names of the Star Fleet captains central to the _Star Trek_ t.v. shows, the actor who portrayed that character, and the year the series first appeared.

```{r}

tribble(
  ~"lastname", ~"actor", ~"first_appeared",
...
There are similar ways to make a `data.frame()`
