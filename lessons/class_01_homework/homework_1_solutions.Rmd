...
# Solution
mtcars <- read_excel(datasets, "mtcars")
...
>* GST in dollars
>
> Think about how the data might be encoded. What sorts of things will you have to watch out for when you read in the data?


Answers:

* Customer names might have letters with accents--will those read ok?

* Addresses: if we have code to split the street name from the type (for example, splitting "Main Street" into "Main" and "Street") in French the street type comes first ("Rue Principale")

* Addresses: spelling of province names and abbreviations differ ("B.C." becomes "C.B.", short for "Colombie-Britannique") -- this will make matching a challenge

* Dates: months may be spelled out, in which case they will be in French.

* Times: what time zone will be used to record the time?

* Costs: what is used to separate the thousands and the decimals? 

* GST: inconsistent across provinces
