...
Here's a data table with Canadian cities (Census Metropolitan Areas) and their populations in 2019.

Our data table contains 2 duplicated records: the entire Vancouver row is repeated, and the Victoria row is repeated but with different population values (the smaller value is the population in 2018).
...
This removes the duplicated Vancouver row, but leaves both of the Victoria rows. 

If we want to preserve only the first of the city and province combinations, we specify the columns in our `distinct()` function:
